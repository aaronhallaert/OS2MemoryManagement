...
public class MemoryController {
	...
	// zorgen dat in de default constructors de juiste dingen qua paging al geset
	// zijn
	...
	public static void instantiate() {
		...
		if (keuze == null) {
			...
			pad = "Instructions_20000_4.xml";
			

		} else if (keuze.equals("20000 i, 20 p")) {
			pad = "Instructions_20000_20.xml";
			

		}

		// String
		// pad="C:\\Users\\tibo\\Documents\\OS2MemoryManagement\\Instructions_30_3.xml";
	...
	public static Map<Integer, List<Integer>> getLRUFramesVanAlleProcessen(
			Map<Integer, List<Integer>> lruFramesPerProces, int aantal) {
	...
	public static void verdeelOverProcessen(List<Integer> vrijgekomenFrames) {
		...
		if (ram.getAantalProcessenAanwezig() == 0) {
			...
			Main.log(Level.INFO, "Er zijn " + vrijgekomenFrames.size()
					+ " frames vrijgekomen en werden verdeeld onder de aanwezige processen");
	 ...
	 * 
	 * @param adres:
	 *            lijst van pagenummer en offset
	 * @param virtueelAdres:
	 *            volledig virtueel adres decimaal
	 * @param pagenummer:
	 *            deel 1 van adres
	 * @param offset:
	 *            deel 2 van adres
	 * @param virtueelAdresLabel:
	 *            GUI label met virtueel adres
	 * @param huidigProces:
	 *            proces van de instructie
	 * @param pte:
	 *            page table entry
	 * @param framenummer
	 * @param reeelAdresLabel:
	 *            Gui label met reeel adres
	 * @param f:
	 *            Frame waar naar geschreven moet worden
	 */
	public static void writeMethod(List<Integer> adres, int virtueelAdres, int pagenummer, int offset,
			Label virtueelAdresLabel, Proces huidigProces, PTEntry pte, int framenummer, Label reeelAdresLabel,
			Frame f) {
		...
		Main.log(Level.INFO,
				"Proces " + huidigProces.getPid() + " write naar page " + pagenummer + " met offset " + offset);
		...
		// als de pagina niet aanwezig is --> swapping nodig
			...
			// als de pagetable al een entry heeft gehad : de pagina is al gealloceerd
			// geweest in RAM
			// ---> pagetable aanpassen
		...
		for (int i = 0; i < huidigProces.getPageTable().size(); i++) {
			if (huidigProces.getPageTable().get(i).isPresent()) {
				...
				if (huidigProces.getPageTable().get(i).isModified()) {
					...
					Main.log(Level.INFO,
							"Frame " + huidigProces.getPageTable().get(i).getFrameNr()
									+ " is aangepast en wordt weggeschreven naar page "
									+ huidigProces.getPageTable().get(i).getPageNr());
					huidigProces.schrijfNaarVM(
							ram.getFrame(huidigProces.getPageTable().get(i).getFrameNr()).getGeheugenPlaatsen(),
							huidigProces.getPageTable().get(i).getPageNr());
		...
		for (int i = 0; i < lruFramesPerProces.size(); i++) {
			...
			for (int j = 1; j < lruFramesVanProces.size(); j++) {
				...
				if (pteProces.isModified()) {
					...
					Main.log(Level.INFO,
							"Er hoeft niets weggeschreven te worden, page " + pteProces.getPageNr() + " van proces "
									+ huidigProces.getPid() + " in frame " + pteProces.getFrameNr()
									+ " werd niet aangepast");
				}
	 ...
	 *            Gui label met reeel adres
	 */
	public static void readMethod(List<Integer> adres, int pagenummer, int offset, Proces huidigProces, int pid,
			int virtueelAdres, Label virtueelAdresLabel, PTEntry pte, int framenummer, Frame f, Label reeelAdresLabel) {
		...
		virtueelAdresLabel.setText("Page " + pagenummer + "\nOffset " + offset);
		// checken als de pagina die we willen lezen van het huidige proces al aanwezig
		// is in ram
		if (!huidigProces.getPageTable().get(pagenummer).isPresent()) {

			// als de pagina niet aanwezig is, eerst swappen voor de LRU pagina die wel al
			// aanwezig is in het proces
			int frameNummerWeg = getLRUFrameVanProces(huidigProces.getPid());
			int pageNummerWeg = huidigProces.getPageIdByFrameNummer(frameNummerWeg);

			// bij het wegschrijven moet deze pagina enkel upgedate worden naar VM als hij
			// aangepast is
			if (huidigProces.getPageTable().get(pageNummerWeg).isModified()) {
				// oude pagina wegschrijven
				huidigProces.schrijfNaarVM(ram.getFrame(frameNummerWeg).getGeheugenPlaatsen(), pageNummerWeg);
			...
			Main.log(Level.INFO, "Page is niet aanwezig in RAM, werd geswapt met frame " + frameNummerWeg
					+ "corresponderend met page " + pageNummerWeg + " van proces " + pid + " (het huidig proces)");

		} else {
			Main.log(Level.INFO,
					"page is aanwezig in ram op frame " + huidigProces.getPageTable().get(pagenummer).getFrameNr());
		}
